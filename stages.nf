...
workflow CT {
    ...
    txts_ch = channel.fromPath("${params.data.path}/${txts_glob}", glob: true, checkIfExists: true)
    // fullband_di_smooth_data_SB???.txt"

    msouts_ch = txts_ch.collect { it.getSimpleName().replace(di_ms_stem, bp_ms_stem) + '.MS' }
    // //"fullband_di_smooth", "di_bandpass")
...
workflow DIBandpass {
    ...
    //params.ddecal.beam.outcol )

    im_names_ch = mset_ch.collect { it.getSimpleName() + "_" + params.wsclean.imname }

    mset_and_names_ch = mset_ch.merge(im_names_ch.flatten())

    WScleanImage(aoq_ch, mset_and_names_ch, params.wsclean.size, params.wsclean.scale, params.wsclean.niter, params.wsclean.pol, params.wsclean.chansout_per_subband, params.wsclean.minuvl, params.wsclean.maxuvl, params.wsclean.weight, params.wsclean.polfit, params.ddecal.bp.outcol)
...
workflow AVG {
    ...
    averaged_msnames_ch = mset_ch.collect { it.getName() + '.dd_averaged' }
    //.replace( "_002", "_003" ) }

    all_msets_and_averaged_msnames_ch = mset_ch.flatten().merge(averaged_msnames_ch.flatten())

    Average(start_ch, all_msets_and_averaged_msnames_ch, params.ddecal.bp.outcol, params.average.ditodd.timestep, params.average.ditodd.freqstep)
...
workflow DD {
    ...
    calibrate_ch = DP3CalibrateDD(
        start_ch,
        mset_ch,
        params.ddecal.dd.parset,
        params.ddecal.dd.sourcedb,
        params.ddecal.dd.sols,
        params.ddecal.dd.incol,
        params.ddecal.dd.calmode,
        params.ddecal.dd.solint,
        params.ddecal.dd.uvlambdamin,
        params.ddecal.dd.uvlambdamax,
        params.ddecal.dd.uvmmax,
        params.ddecal.dd.nchan,
        params.ddecal.dd.usebeam,
        params.ddecal.dd.beammode,
        params.ddecal.dd.smoothnessconstraint,
        params.ddecal.dd.truncateksmoothkernel,
        params.ddecal.dd.robust_reg,
        params.ddecal.dd.propagate_sols,
        params.ddecal.dd.maxiter,
        params.ddecal.dd.beamproximitylimit,
        params.ddecal.dd.correctfreqsmearing,
        params.ddecal.dd.flagstations,
        params.ddecal.dd.propagate_converged_sols_only,
        0.15,
        1,
        'directioniterative',
    )
...
workflow WS {
    ...
    WScleanImage(start_ch, mset_and_names_ch, params.wsclean.size, params.wsclean.scale, params.wsclean.niter, params.wsclean.pol, params.wsclean.chansout, params.wsclean.minuvl, params.wsclean.maxuvl, params.wsclean.weight, params.wsclean.polfit, params.wsclean.column)
}
